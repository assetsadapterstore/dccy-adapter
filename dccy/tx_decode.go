...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	codeAccount = addr[0]
	publisher := addr[1]
	tokenCoin = strings.ToUpper(addr[2])
	...
	accountAssets, err := decoder.wm.GetCurrencyBalance(eos.AccountName(account.Alias), tokenCoin, eos.AccountName(codeAccount), publisher)
...
func (decoder *TransactionDecoder) createRawTransaction(
	...
	addr := strings.Split(rawTx.Coin.Contract.Address, ":")
	if len(addr) != 3 {
		return openwallet.Errorf(openwallet.ErrCreateRawTransactionFailed,"token contract's address is invalid: %s", rawTx.Coin.Contract.Address)
	}
	codeAccount := eos.AccountName(addr[0])
	...
	publisher := addr[1]
	action := NewExTransfer(accountResp.AccountName, to, quantity, memo, publisher)
