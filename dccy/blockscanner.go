...
//DCCYBlockScanner DCCY block scanner
type DCCYBlockScanner struct {
...
// NewDCCYBlockScanner create a block scanner
func NewDCCYBlockScanner(wm *WalletManager) *DCCYBlockScanner {
	bs := DCCYBlockScanner{
...
func (bs *DCCYBlockScanner) ScanBlockTask() {
...
func (bs *DCCYBlockScanner) forkBlockNotify(block *Block) {
...
//newBlockNotify 获得新区块后，通知给观测者
func (bs *DCCYBlockScanner) newBlockNotify(block *eos.BlockResp) {
	header := ParseHeader(block)
	bs.NewBlockNotify(header)
}

// BatchExtractTransactions 批量提取交易单
func (bs *DCCYBlockScanner) BatchExtractTransactions(blockHeight uint64, blockHash string, blockTime int64, transactions []eos.TransactionReceipt) error {
...
func (bs *DCCYBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *DCCYBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, blockTime int64, transaction eos.TransactionReceipt, scanTargetFunc openwallet.BlockScanTargetFunc) ExtractResult {
	var (
		success = true
	...
	for _, action := range signedTransaction.Actions {
		...
		if _, exist := bs.MonitorActions[string(action.Name)]; exist {
			...
			jsonMsg := gjson.ParseBytes(bytes)
			switch string(action.Name) {
			case "transfer":
				asset, _ := eos.NewAsset(jsonMsg.Get("quantity").String())
				data = TransferData{
					From:       jsonMsg.Get("from").String(),
					To:         jsonMsg.Get("to").String(),
					Quantity:   asset,
					Memo:       jsonMsg.Get("memo").String(),
					ActionName: string(action.Name),
					Contract:   "eosio",
				}
			case "extransfer":
				asset, _ := eos.NewAsset(jsonMsg.Get("quantity.quantity").String())
				data = TransferData{
					From:       jsonMsg.Get("from").String(),
					To:         jsonMsg.Get("to").String(),
					Quantity:   asset,
					Memo:       jsonMsg.Get("memo").String(),
					ActionName: string(action.Name),
					Contract:   jsonMsg.Get("quantity.contract").String(),
				}
...
func (bs *DCCYBlockScanner) InitExtractResult(sourceKey string, action TransferAction, result *ExtractResult, optType int64) {
	...
	contractID := openwallet.GenContractID(bs.wm.Symbol(), string(action.Account)+":"+action.Contract+":"+symbol)
	...
	coin.Contract = openwallet.SmartContract{
		Symbol:     bs.wm.Symbol(),
		ContractID: contractID,
		Address:    string(action.Account) + ":" + action.Contract + ":" + symbol,
		Token:      symbol,
	}
...
func (bs *DCCYBlockScanner) extractTxInput(action TransferAction, txExtractData *openwallet.TxExtractData) {
...
func (bs *DCCYBlockScanner) extractTxOutput(action TransferAction, txExtractData *openwallet.TxExtractData) {
...
func (bs *DCCYBlockScanner) newExtractDataNotify(height uint64, extractData map[string][]*openwallet.TxExtractData) error {
	if extractData == nil {
		return nil
	}
...
//ScanBlock 扫描指定高度区块
func (bs *DCCYBlockScanner) ScanBlock(height uint64) error {
...
func (bs *DCCYBlockScanner) scanBlock(height uint64) (*eos.BlockResp, error) {
...
func (bs *DCCYBlockScanner) SetRescanBlockHeight(height uint64) error {
	if height <= 0 {
...
func (bs *DCCYBlockScanner) GetGlobalMaxBlockHeight() uint64 {
	headBlock, err := bs.GetGlobalHeadBlock()
	if err != nil {
...
func (bs *DCCYBlockScanner) GetGlobalHeadBlock() (block *eos.BlockResp, err error) {
	infoResp, err := bs.GetChainInfo()
	if err != nil {
...
func (bs *DCCYBlockScanner) GetChainInfo() (infoResp *eos.InfoResp, err error) {
...
func (bs *DCCYBlockScanner) GetScannedBlockHeight() uint64 {
	height, _, _ := bs.GetLocalBlockHead()
	return uint64(height)
}

//GetBalanceByAddress 查询地址余额
func (bs *DCCYBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
...
func (bs *DCCYBlockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
	infoResp, err := bs.GetChainInfo()
	if err != nil {
		bs.wm.Log.Std.Info("get chain info error;unexpected error:%v", err)
		return nil, err
...
func (bs *DCCYBlockScanner) RescanFailedRecord() {
...
//@optional
func (bs *DCCYBlockScanner) SupportBlockchainDAI() bool {
	return true
}
