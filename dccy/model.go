...
func NewExTransfer(from, to eos.AccountName, quantity eos.Asset, memo string, contract string) *eos.Action {
	return &eos.Action{
		Account: eos.AN("eosio.token"),
		Name:    eos.ActN("extransfer"),
		...
		ActionData: eos.NewActionData(ExTransfer{
			From: from,
			To:   to,
			Quantity: eos.ExtendedAsset{
				Asset:    quantity,
				Contract: eos.AN(contract),
			},
			Memo: memo,
		}),
	}
...
// Transfer represents the `extransfer` struct on `eosio.token` contract.
