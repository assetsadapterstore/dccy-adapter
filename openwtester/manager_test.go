...
func TestWalletManager_CreateWallet(t *testing.T) {
	...
	w := &openwallet.Wallet{Alias: "HELLO DCCY", IsTrust: true, Password: "12345678"}
...
func TestWalletManager_GetWalletInfo(t *testing.T) {
	...
	wallet, err := tm.GetWalletInfo(testApp, "WDPbcNXUjpbYSjDZRLSxXoBVKeFG3oVcb9")
	if err != nil {
...
func TestWalletManager_CreateAssetsAccount(t *testing.T) {
	...
	dccyount := &openwallet.AssetsAccount{Alias: "testopendccy", WalletID: walletID, Required: 1, Symbol: "DCCY", IsTrust: true}
	dccyount, address, err := tm.CreateAssetsAccount(testApp, walletID, "12345678", dccyount, nil)
	if err != nil {
		log.Error(err)
		return
	}

	log.Info("dccyount:", dccyount)
...
func TestWalletManager_GetAssetsAccountList(t *testing.T) {
	...
	for i, w := range list {
		log.Info("dccyount[", i, "] :", w)
	}
	log.Info("dccyount count:", len(list))
...
func TestWalletManager_CreateAddress(t *testing.T) {
	...
	walletID := "WDPbcNXUjpbYSjDZRLSxXoBVKeFG3oVcb9"
	dccyountID := "Ag6ifKv8qkBShPb37TUAY77vZHrmDmb2xEXCjqyutznQ"
	address, err := tm.CreateAddress(testApp, walletID, dccyountID, 1)
	if err != nil {
		log.Error(err)
		return
	}
